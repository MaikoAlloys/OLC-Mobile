...
const HODFeedbacks = () => {
  ...
  useEffect(() => {
    // Fetch HOD data and feedback when component mounts
    const fetchData = async () => {
      try {
        // Get HOD data from AsyncStorage
        const hodData = await AsyncStorage.getItem("hodToken");
        if (hodData) {
          const parsedData = JSON.parse(hodData);
          setHodName(parsedData.username); // Set HOD name for replies
          fetchFeedback();
        } else {
          setLoading(false);
        ...
        setLoading(false);
        Alert.alert('Error', 'Failed to load HOD data');
      }
    };
  ...
  const fetchFeedback = async () => {
    try {
      setLoading(true);
      const response = await api.get('/feedback/hod-feedback');
  ...
  const handleReplySubmit = async () => {
    ...
    try {
      await api.post(`/feedback/hod-feedback/reply/${replyingTo.feedback_id}`, {
        replyMessage: replyMessage.trim(),
        replyBy: hodName.trim() // Auto-filled from HOD data
      });
  ...
  const renderFeedbackItem = ({ item }) => (
    <View style={styles.feedbackItem}>
      <View style={styles.feedbackHeader}>
        <View>
          <Text style={styles.feedbackTitle}>{item.subject || 'Department Inquiry'}</Text>
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Department Feedback</Text>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <Text style={styles.modalSubject}>{replyingTo?.subject || 'Department Inquiry'}</Text>
